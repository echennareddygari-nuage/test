...
def main():
    ...
    # Set some starting values for variables
    now = datetime.utcnow()
    json_dict['daysLeft'] = 9999999
    json_dict['expiryDate'] = "UNK"
    json_dict['daysLeftNsg'] = 9999999
    json_dict['expiryDateNsg'] = "UNK"

    for nc_license in nc.user.licenses.get():

        # get VSP license info
        ...
        # get CPE license info
        if nc_license.allowed_cpes_count > 0:

            # Extract the next CPE license to expire and its associated info
            expiryDate = datetime.utcfromtimestamp(nc_license.expiration_date/1000)
            timeDelta = (expiryDate - now)

            # only keep the closest expiry data
            if timeDelta.days < json_dict['daysLeftNsg']:

                # overwrite previous values with this one
                json_dict['daysLeftNsg'] = timeDelta.days
                json_dict['expiryDateNsg'] = expiryDate.strftime("%Y-%m-%d %H:%M:%SZ")
